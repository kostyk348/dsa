...
static void map_set(unsigned long long ptr, unsigned long long off){
    ...
    for(unsigned k=0;k<HSIZE;k++){ unsigned j=(i+k)%HSIZE;
        if(h_ptr[j]==0){ h_ptr[j]=ptr; h_off[j]=off; h_wr[j]=0; return; }
        if(h_ptr[j]==ptr){ return; } }
}
...
static int looks_plain(const unsigned char*s, size_t n){
    size_t m=n>512?512:n; if(m<16) return 0;
    int pr=0;
    for(size_t i=0;i<m;i++){ unsigned c=s[i]; if(c==0) { if(i>16) return pr>(int)(m*0.5); } else if((c>=0x20&&c<0x7f)||c=='\n'||c=='\t'||c=='\r') pr++; }
    return pr>(int)(m*0.55);
}
static int h_wr_at(unsigned long long);
...
static void handle(const void *src, void *dst, size_t n){
    ...
    if(g_mmap_base==0 && n>=16 && memcmp(s,g_ct,8)==0){ g_mmap_base=su; logmsg("MMAP_BASE=%llx\n", g_mmap_base); }
    if(g_mmap_base && n>=16){
        unsigned long long off=su-g_mmap_base;
        if(su>=g_mmap_base && off<g_filesize) map_set(du,off);
    }
    if(g_outfd<0) g_outfd=open("/tmp/ojd_full.bin",O_WRONLY|O_CREAT|O_TRUNC,0644);
    unsigned long long off;
    if(map_get(su,&off)){
        int do_dump = (off==0) ? (n>=16 && memcmp(s,"SQLite format 3",15)==0) : 1;
        if(do_dump){
            pwrite(g_outfd, src, n, (off_t)off);
            logmsg("PLAIN off=%llu n=%lu\n", off, (unsigned long)n);
        }
        if(n>=16) map_set(du,off); // chain decrypt output (last copy wins)
    }
}
